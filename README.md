...
The functions: $ \vec{u}_{h, \sigma}(t, \vec{x}_i(t))$ and $\omega_{h, \sigma}(t, \vec{x}_i(t))$ are the interpolator, that are used to evaluate the field where no particles are present.
